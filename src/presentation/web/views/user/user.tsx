...
export default function UserView({ users, user }: { users: IGenericPaginatedData<IUser>; user: IUserCreate }) {
  ...
  return (
    <div className="p-4 bg-cyan-950 text-gray-200 h-full">
      ...
      <form onSubmit={handleFormSubmit} className="max-w-xl text-gray-700 bg-gray-300 p-4 rounded-lg mb-4 mt-4">
      ...
      <table id="userTable" className="border-spacing-0 border-t border-l max-w-xl">
        ...
        <tbody>
          {users.data.map((item) => (
            <tr key={item.id}>
              <td className="border-r border-b px-4 py-2 whitespace-nowrap">{item.fullname}</td>
              <td className="border-r border-b px-4 py-2 whitespace-nowrap">{item.username}</td>
      ...
      Showing from {users.from} to {users.to} of
